### --------------------------------------------------------------------------
### R Code: univariate LSTM for life expectancy forecasting
### --------------------------------------------------------------------------
### 
### R version 3.6.3
...
# Uploading Japan Males life expectancy time series, both for x=0 and x=65
#----------------------------------------------------------------------
data=read.csv2("Life_exp_data.csv")


###---------------------------------------------------------------------------###
###           Working on life expectancy time series for x=0                  ###
###---------------------------------------------------------------------------###
...
supervised_le_x0 = as.vector(data$x.0)
for(i in 1:numberlag) {
  lag_le_x0 = c(rep(as.vector(data$x.0)[1],i),as.vector(data$x.0)[1:(length(data$x.0)-i)])
  supervised_le_x0 = cbind(supervised_le_x0,lag_le_x0)}
train_le_x0 = supervised_le_x0[1:n, ]
test_le_x0 = supervised_le_x0[(n+1):nrow(supervised_le_x0),]
x_train_le_x0 = train_le_x0[,-1]
y_train_le_x0 = train_le_x0[,1]
x_test_le_x0 = test_le_x0[,-1]
y_test_le_x0 = test_le_x0[,1]
...
dim(x_train_le_x0) <- c(n, (numberlag), 1)
X_shape2 = numberlag
X_shape3 = 1
batch_size = 1
...
tuning = expand.grid(epochs = c(seq(1,20,1)), unit = c(seq(1,20,1))) 
...
system.time(for (g in 1:(nrow(tuning))){
  ...
  tuning$performance[g] = rmse(predict_test, y_test_le_x0)
})
...
rmse(predict_test, y_test_le_x0)
mae(predict_test, y_test_le_x0)
...
###---------------------------------------------------------------------------###
###           Working on life expectancy time series for x=65                 ###
###---------------------------------------------------------------------------###

supervised_le_x65 = as.vector(data$x.65)
for(i in 1:numberlag) {
  lag_le_x65 = c(rep(as.vector(data$x.65)[1],i),as.vector(data$x.65)[1:(length(data$x.65)-i)])
  supervised_le_x65 = cbind(supervised_le_x65,lag_le_x65)}
train_le_x65 = supervised_le_x65[1:n, ]
test_le_x65 = supervised_le_x65[(n+1):nrow(supervised_le_x65),]
x_train_le_x65 = train_le_x65[,-1]
y_train_le_x65 = train_le_x65[,1]
x_test_le_x65 = test_le_x65[,-1]
y_test_le_x65 = test_le_x65[,1]


# Neural Network's tuning
#------------------------------------------------

dim(x_train_le_x65) <- c(n, (numberlag), 1)
X_shape2 = numberlag
X_shape3 = 1
batch_size = 1
...
system.time(for (g in 1:(nrow(tuning))){
  ...
  tuning$performance[g] = rmse(predict_test, y_test_le_x65)
})
...
rmse(predict_test, y_test_le_x65)
mae(predict_test, y_test_le_x65)
